...
export const getUserConversation = async (
  ...
  try {
    // user check not needed, because it's a protected function that only a logged in user can access
...
export const createConversation = async (
  ...
  try {
    ...
    // converts array elements to type number, then removes duplicates
    ...
    // the array of users ids must contain the id of the person creating a conversation
    ...
    // admin ids array must contain the id of the person creating a group conversation (with more than 2 users)
    ...
    // merged array of user ids and admin ids (so there are no duplicates)
    const mergedArray = removeDuplicatesFromArrays(userIdsInt, adminIdsInt);

    // checks if all user and admin ids actually exist
    ...
    // there's no admins and 'group === false' if only two users are in a conversation
...
export const updateTitleAndImage = async (
  ...
  try {
    ...
    // no need to check if there are more than 2 users in the group
    // there are no admins in a conversation with 2 users
...
export const leaveGroup = async (
  ...
  try {
    ...
    // admin should not be able to leave a group if he's the only admin left
    // and there are still other non-admin users there
...
export const removeUserAdminStatus = async (
  ...
  try {
    ...
    // this one is most likely not needed since admins cannot remove themselves
